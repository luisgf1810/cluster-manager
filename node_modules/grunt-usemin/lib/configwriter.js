...
//
// Create a test writer for the furnished flow.
// The created test will:
...
// under the same name, in test directory.
//
...
//
//   We'll:
//     - have files test/concat.js and test/uglifyjs.js implementing each test writer
...
var ConfigWriter = module.exports = function (flow, dirs) {
  ...
  // We need to create all the needed test writers, given them their output directory
  ...
  flow.blockTypes().forEach(function (blockType) {
    ...
    flow.steps(blockType).forEach(function (item) {
      var step;
      if (_.isString(item)) {
        step = require(path.join(__dirname, 'test', item));
      } else {
    ...
    flow.post(blockType).forEach(function (item) {
      var pp;
      if (_.isString(item)) {
        pp = require(path.join(__dirname, 'test', item));
      } else {
...
ConfigWriter.prototype.process = function (file, config) {
  ...
  lfile.blocks.forEach(function (block) {
    ...
    self.forEachStep(block.type, function (writer, last) {
      ...
      // test[writer.name].generated = _.extend(test[writer.name].generated, writer.createConfig(context, block));
