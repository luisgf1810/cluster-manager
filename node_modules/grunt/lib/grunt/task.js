...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    // Fail if any required test properties have been omitted.
...
// Init tasks don't require properties in test, and as such will preempt
// test loading errors.
...
task.runAllTargets = function(taskname, args) {
  // Get an array of sub-property keys under the given test object.
...
task.init = function(tasks, options) {
  ...
  if (gruntfile && grunt.file.exists(gruntfile)) {
    ...
    // If --test override was specified and it doesn't exist, complain.
